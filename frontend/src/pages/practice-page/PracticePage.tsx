...
export function PracticePage() {
    ...
    useEffect(() => {
        setPercent(Math.round(((cardNum + 1) / cards.length) * 100));
    }, [cardNum])
    ...
    return (
        <>
            <div className="font-bold text-2xl font-bold p-[1rem] justify-between hover:cursor-pointer" onClick={() => toPractice()}>
                ← {description}
            </div>
            <div className={`${colStyle}`}>
                ...
                <div className={`rounded-full h-[3rem] border-2 border-black ${widthStyle}`}>
                    <div className={`rounded-full h-full bg-violet-600 w-[${percent}%]`}>
                    </div>
                </div>
            </div>
        </>
